...
public class ApiFactoryProcess extends AbstractProcessor {
...
//                        ClassName apiUtil = ClassName.get("com.base.util", "ApiUtil");
                        ClassName C = ClassName.get("com.hbck.hospital.api", "C");
...
//                                    blockBuilder.add("$L.getSkip(param)" + split, apiUtil);
                        ...
                        methodBuilder.addStatement(
                                "return $T.getInstance()" +
                                        ".service.$L($L)" +
                                        ".compose($T.io_main())"
                                , ClassName.get("com.hbck.hospital.api", "Api")
                                , e.getSimpleName().toString()
                                , blockBuilder.build().toString()
                                , ClassName.get("com.hbck.hospital.base.util", "RxSchedulers"));
                        ...
                        methodBuilder.addStatement(
                                ...
                                , ClassName.get("com.hbck.hospital.api", "Api")
                                , e.getSimpleName().toString()
                                , paramsString.substring(0, paramsString.length() - 1==-1?0:(paramsString.length() - 1))
                                , ClassName.get("com.hbck.hospital.util", "RxSchedulers"));
                        tb.addMethod(methodBuilder.build());
                    }
                }
            }
            JavaFile javaFile = JavaFile.builder("com.hbck.apt", tb.build()).build();// 生成源代码
